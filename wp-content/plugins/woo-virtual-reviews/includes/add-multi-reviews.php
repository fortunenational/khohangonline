...
class Add_Multi_Reviews {
	...
	public function ajax() {
		if ( ! current_user_can( 'manage_woocommerce' ) || empty( $_POST['sub_action'] ) || empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'wvr_nonce' ) ) {
			wp_send_json_error();
		}
		$func = sanitize_textarea_field( $_POST['sub_action'] );
	...
	public function add_multiple_reviews() {
		$cats    = ! empty( $_POST['cats'] ) ? wc_clean( $_POST['cats'] ) : [];
		$step    = ! empty( $_POST['step'] ) ? sanitize_text_field( $_POST['step'] ) : 1;
		$cmt_qty = ! empty( $_POST['qty'] ) ? sanitize_text_field( $_POST['qty'] ) : 1;
	...
	public function add_reviews( $pid, $qty, $from, $to ) {
		...
		$random_time_arr = [];
		for ( $i = 0; $i < $qty; $i ++ ) {
			$random_time_arr[] = $from + rand( 0, $count ) * HOUR_IN_SECONDS;
		}
		sort( $random_time_arr );

		$settings = Data::instance();
		$names    = $settings->get_param( 'names' );
		$comments = $settings->get_param( 'cmt' );
	...
	function add_reviews_by_submit() {
		...
		$_REQUEST['_wp_http_referer'] = remove_query_arg( 'wvr_error', $_REQUEST['_wp_http_referer'] );

		$post_ids = wc_clean( $_REQUEST['post'] );
		$qty      = isset( $_REQUEST['wvr_select_qty_cmt'] ) ? sanitize_text_field( $_REQUEST['wvr_select_qty_cmt'] ) : null;
		...
		if ( $max > $limit || $qty > $limit ) {
			$_REQUEST['_wp_http_referer'] = add_query_arg( 'wvr_error', 'quantity', $_REQUEST['_wp_http_referer'] );
		...
		$from = ! empty( $_REQUEST['wvr_date_from'] ) ? sanitize_text_field( $_REQUEST['wvr_date_from'] ) : 'today';
		$to   = ! empty( $_REQUEST['wvr_date_to'] ) ? sanitize_text_field( $_REQUEST['wvr_date_to'] ) : 'today';
		$from = strtotime( $from );
		$to   = strtotime( $to ) + DAY_IN_SECONDS - 1;

		if ( ! empty( $post_ids ) && is_array( $post_ids ) ) {
			foreach ( $post_ids as $pid ) {
				$this->add_reviews( $pid, rand( $min, $max ), $from, $to );
			}
		}
	}
	...
	public function show_virtual_review_count( $column_name, $pid ) {
		if ( $column_name == 'virtual_review' ) {
			...
			echo $rating = get_post_meta( $pid, '_wc_average_rating', true );
		}
	}
