    ...
    <style>
            ...
            */
            ...
            <?php foreach ( wpcargo_package_fields() as $key => $value ): ?>
				...
				/* Shipping rate pacakge */
				#wpcsr-rate-packages table.wpcargo-table td:nth-of-type(<?php echo $counter; ?>):before { content: "<?php echo $value['label']; ?>"; }
				/* Normal package */
                #wpc-multiple-package table.wpcargo-table td:nth-of-type(<?php echo $counter; ?>):before { content: "<?php echo $value['label']; ?>"; }
		...
		<div class="wpcargo-row">
			<?php if( $dim_enable ): ?>
				<section class="<?php echo $class; ?> first" style="text-align: center; font-weight: bold;">
					<?php echo apply_filters( 'wpcargo_package_volumetric_label', esc_html__('Total Volumetric Weight :', 'wpcargo') ); ?> <span id="package_volumetric"><?php echo $package_volumetric.'</span>'.wpcargo_package_settings()->weight_unit; ?>.
				</section>
				<section class="<?php echo $class; ?>" style="text-align: center; font-weight: bold;">
					<?php echo apply_filters( 'wpcargo_package_volume_label', esc_html__('Total Volume :', 'wpcargo') ); ?> <span id="package_volume"><?php echo $package_volume.'</span>'.wpcargo_volume_unit_label(); ?>
				</section>
			<?php endif; ?>
			<section class="<?php echo $class; ?>" style="text-align: center; font-weight: bold;">
...
function wpcargo_after_package_details_script_callback( $shipment ){
	...
	<script>
		...
		var cubic_divisor 	= <?php echo $cubic_divisor; ?>;  
		var cubic_operator 	= "<?php echo $cubic_operator; ?>";  
		jQuery(document).ready(function($){
			<?php do_action( 'wpcargo_package_details_script' ); ?>
			function getDecimal( value = 0 ){
				value = parseFloat( value );
				if( value >= 1 ){
					return <?php echo $decimal; ?>;
				}
				var str 	= "0.";
				var dPlace = 1;
				for (i = 1; i < 10; i++) {
