...
class WPC_MP_Settings {
	...
	public function wpcargo_dim_divisor_render(){
		...
		?>
		<input type='number' name='wpc_mp_settings[wpcargo_dim_divisor]' value="<?php echo array_key_exists( 'wpcargo_dim_divisor', $options ) ? $options['wpcargo_dim_divisor'] : '' ; ?>">
	...
	public function wpc_mp_dimension_unit_render() {
		...
		<select name="wpc_mp_settings[wpc_mp_dimension_unit]" id="wpc_mp_dimension_unit" required>
			<?php foreach( wpcargo_package_dim_units() as $dim_unit): ?>
				<option value="<?php echo $dim_unit; ?>" <?php selected( $unit, $dim_unit, true ); ?>><?php echo $dim_unit; ?></option>
			<?php endforeach; ?>
		</select>
